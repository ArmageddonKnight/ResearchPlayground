...
class PegasusAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            assert not torch.isnan(key_states).any(), f"key_states={key_states} has NANs"
            assert not torch.isnan(value_states).any(), f"value_states={value_states} has NANs"

        elif past_key_value is not None:
            # reuse k, v, self_attention

            assert not torch.isnan(past_key_value[0]).any(), f"past_key_value={past_key_value} has NANs"
            assert not torch.isnan(past_key_value[1]).any(), f"past_key_value={past_key_value} has NANs"
        ...
        assert not torch.isnan(query_states).any(), f"query_states={query_states} has NANs"
        assert not torch.isnan(key_states).any(), f"key_states={key_states} has NANs"

        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))

        assert not torch.isnan(attn_weights).any(), f"attn_weights={attn_weights} has NANs"
        ...
        if int(os.getenv("DEBUG_CUR_LEN", "0")) == 2 and int(os.getenv("DEBUG_LAYER_IDX", "0")) == 1:
            from quik_fix import negative
            print(negative(attn_weights))
        assert not torch.isinf(attn_weights).any(), f"attn_weights={attn_weights} has INFs"

        old_attn_weights = attn_weights.clone().detach()

        attn_weights = nn.functional.softmax(attn_weights, dim=-1)

        assert not torch.isnan(attn_weights).any(), f"attn_weights={attn_weights} has NANs, old_attn_weights={old_attn_weights}"
        ...
        assert not torch.isnan(attn_probs).any(), f"attn_probs={attn_probs} has NANs"

        attn_output = torch.bmm(attn_probs, value_states)

        assert not torch.isnan(attn_output).any(), f"attn_output={attn_output} has NANs"
...
class PegasusDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            assert not torch.isnan(hidden_states).any(), f"hidden_states={hidden_states} has NANs"
            ...
            assert not torch.isnan(hidden_states).any(), f"hidden_states={hidden_states} has NANs"
            assert not torch.isnan(residual).any(), f"hidden_states={residual} has NANs"
...
class PegasusDecoder(PegasusPreTrainedModel):
    ...
    def forward(
        ...
        # if int(os.getenv("DEBUG_CUR_LEN", "0")) == 2:
        #     for past_idx, past_kv in enumerate(past_key_values):
        #         for vi, v in enumerate(past_kv):
        #             print(f"{past_idx}, {vi}, past_key_values={past_key_values[past_idx][vi]}")
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            from quik_fix import negative
            print(negative(f"layer_idx={idx}"))
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if idx == 0:
                #     from quik_fix import positive
                #     print(f"layer_outputs_0={positive(layer_outputs[0])}")

            hidden_states = layer_outputs[0]

            # assert not torch.isnan(hidden_states).any(), f"hidden_states={hidden_states} @decoder layer ({idx}) has NANs"
...
class PegasusForConditionalGeneration(PegasusPreTrainedModel):
    ...
    def forward(
        ...
        # assert not torch.isnan(lm_logits).any(), "Has NAN!"
    ...
    def _reorder_cache(past, beam_idx):
        ...
        if int(os.getenv("DECODER_COMPILED_WITH_MODULE_ARGS_INLINED", "0")):
            ...
            # print(f"Before reordering={past[1][0]}")
            ...
            # print(f"After reordering={reordered_past[1][0]}")
